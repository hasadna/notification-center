    ...
    def post(self):
        ...
        if not user:
            ...
            subscription = Subscription( user = user, source = src.key, 
                                         tags = tags, latlon = latlon, period = period, 
...
class PollRssHandler(webapp2.RequestHandler):
    ...
    def send_mail(self,subscription,feed,maxpublished):
        ...
        logging.log(logging.ERROR,"%r,%r" % (subscription,to_send))
        ...
        mail.send_mail(sender="Hasadna Notification Center <%s>" % sender,
                       ...
                       body="%s\n\n" % feed.feed.subtitle + "\n---\n".join(x.description for x in to_send),
                       html="<h2>%s</h2>" % feed.feed.subtitle + "<hr/>".join(x.description for x in to_send)
                       )
